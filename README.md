...
- Scroll-wheel history display — browse and reuse past calculations
...
- Formula evaluation via AZFormula

## Architecture

```
Calclin/
├── App/              — App entry point, global config (AZDecimalConfig)
├── CalcView/         — Main calculator screen (CalcViewModel)
├── KeyboardView/     — Custom keyboard (JSON-driven)
└── SettingView/      — Settings (rounding, grouping, separators)
```
